module Fun.SunUI.HotReload.Utils
...
let internal reload<'UIStack, 'T> renderEntryName (codeData: (string * DFile)[]) (updateRenderFn: ('T -> ElementCreator<'UIStack>) -> unit) =
    ...
    let unsupport () =
        ...
        printfn "***   FAIL: the declaration had the wrong type. it must be a single top-level value in a module."
                            ...
                            |> Array.tryPick (fun (_, file) ->
                                ...
                                | Some (membDef, _) ->
                                    ...
                                    if membDef.Parameters.Length > 0 then
                                        ...
                                        | :? ElementCreator<'UIStack> as render ->
